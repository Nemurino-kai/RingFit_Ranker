...
def index():
    ...
    pagination = Pagination(page=page, total=len(
        exercise_data_list), per_page=100, css_framework='bootstrap4')
...
def analytics():
    ...
    if day is not None:
        ...
        start_day = day
        stop_day = datetime.datetime.strftime(datetime.datetime.strptime(
            day, '%Y-%m-%d') + datetime.timedelta(days=1), '%Y-%m-%d')
